...
class GAIL:
    ...
    def policy_loss(self, data):
        ...
        loss = -torch.mean(torch.log(discriminator_output))
    ...
    def fit_discriminator(self, expert_data, generated_data, suppress_fit_tqdm=False):
        ...
        for ep in config_tqdm(range(self.epochs), suppress_fit_tqdm):
            for mb in range(int(expert_data["observations"].shape[0] / self.mb_size)):
                self.optimizer_discriminator.zero_grad()

                # 随机选择小批量的索引
                rand_idx_expert = np.random.choice(expert_data["observations"].shape[0], size=self.mb_size)
                rand_idx_generated = np.random.choice(generated_data["observations"].shape[0], size=self.mb_size)
    ...
    def fit_policy(self, data, suppress_fit_tqdm=False):
        # 验证数据是否包含必要的键
        ...
        # 如果要保存日志，记录训练前的记录
        if self.save_logs:
            loss_val = self.policy_loss(data).data.numpy().ravel()[0]
            self.policy_logger.log_kv('loss_before', loss_val)

        # 训练策略循环
        for ep in config_tqdm(range(self.epochs), suppress_fit_tqdm):
            for mb in range(int(data["observations"].shape[0] / self.mb_size)):
                self.optimizer_policy.zero_grad()

                rand_idx = np.random.choice(data["observations"].shape[0], size=self.mb_size)
                batch_data_obs = data["observations"][rand_idx]
                batch_actions = data["actions"][rand_idx]
        ...
        # 如果要保存日志，记录训练后的相关信息
        if self.save_logs:
            self.policy_logger.log_kv('epoch', self.epochs)
            loss_val = self.policy_loss(data).data.numpy().ravel()[0]
            self.policy_logger.log_kv('loss_after', loss_val)
            self.policy_logger.log_kv('time', (timer.time() - ts))
    ...
    def train(self, **kwargs):
        ...
        for i in range(self.epochs):
            ...
            self.fit_policy(generated_data)
