...
class NPG(BatchREINFORCE):
    ...
    def train_from_paths(self, paths):
        ...
        observations, actions, advantages, base_stats, self.running_score = self.process_paths(paths)  # 处理路径数据
        if self.save_logs: self.log_rollout_statistics(paths)  # 若需保存日志，记录路径统计信息
        ...
        vpg_grad = self.flat_vpg(observations, actions, advantages)  # VPG 梯度计算
        t_gLL += timer.time() - ts  # 计算 VPG 时间

        # NPG
        ts = timer.time()  # 记录开始时间
        hvp = self.build_Hvp_eval([observations, actions],
                                  regu_coef=self.FIM_invert_args['damping'])  # 构建 HVP
        npg_grad = cg_solve(hvp, vpg_grad, x_0=vpg_grad.copy(),
                            cg_iters=self.FIM_invert_args['iters'])  # NPG 梯度计算
